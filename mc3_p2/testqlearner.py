...
def movebot(data, oldpos, a):
    ...
    # decide if we're going to ignore the action
    # and choose a random one instead
...
def test(map, iterations, learner, verbose):
    ...
    for iteration in range(1, iterations + 1):
        ...
        action = learner.querysetstate(state)  # set state, get action
        count = 0
        while (robopos != goalpos) & (count < 100000):

            # move to new location according to action
            # and then get a new action
